...
1. **Forecast accuracy is only ~25%** across the dataset — a substantial majority of demand predictions are meaningfully off.
2. **80%+ of SKUs are flagged Critical** on any given day — an unusually high rate that points to systemic under-forecasting as a likely root cause, not just poor reordering logic.
3. **Inventory turnover has declined** from ~23 to ~19–20 over the two-year window, suggesting products are moving more slowly over time.
4. Movement tiers (Fast/Medium/Slow) and category/store rollups reveal meaningful variation in performance that a single global inventory policy would miss.
...
1. Import the raw dataset into MySQL as `sales_table`
2. Run scripts in `SQL_Scripts/` in numbered order
3. Open `Power_BI/StockSense_Dashboard.pbix` in Power BI Desktop
